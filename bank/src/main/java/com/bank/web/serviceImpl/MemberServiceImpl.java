...
public class MemberServiceImpl implements MemberService {
	...
	public String remove(String userid) {
		...
		/*
		 * MemberBean tempMember = null; String result = "";
		 * 
		 * if(this.searchById(userid) != null){ for (int i = 0; i <
		 * this.getCount(); i++) { if (memberList[i].getUserid().equals(userid))
		 * { tempMember = memberList[i]; memberList[i] =
		 * memberList[this.getCount()-1]; memberList[this.getCount()-1] = null;
		 * } } this.setCount(this.getCount()-1); result = tempMember.getUserid()
		 * + "삭제 성공"; }else result = "해당 아이디가 없음";
		 * 
		 * return result;
		 */
